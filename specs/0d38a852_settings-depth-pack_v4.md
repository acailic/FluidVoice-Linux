...
5. **Config-test gotchas** (`tests/test_config_settings.py`): :186 asserts
   `RESTART_REQUIRED == {"model.eager_warmup"}` EXACTLY (this plan adds no
   restart/engine keys — keep it that way), and :139/:195 are doc-guard
   tests of the form `assert "<key>" in TEMPLATE` — when Phases 1 and 3 add
   their TEMPLATE comments, extend those doc-guard tests to cover
   `base_prompt` and `languages`. Also note: no `config.example.toml` file
   exists anywhere; the inline `TEMPLATE` string in `config.py` is the only
   example surface (the request's "config example comment" means TEMPLATE).
...
3. `daemon.py`:
   ...
   - `test_dictation` (~line 1197): same substitution —
     `backend.transcribe(Path(wav), backends.effective_language(self.cfg, backend))`.
   - `status` action: add `"active_model_key": backends.backend_model_key(
     self.backend) or backends.config_model_key(self.cfg)`.
   - `cli.py:138` (`fluidvoice transcribe` CLI): same substitution —
     `language=backends.effective_language(cfg, backend)`. (Four language
     call sites total: pipeline, preview, test-dictation, CLI — after the
     change a grep for `general"]["language` must leave only the resolver
     and the backend ctor fallbacks reading it directly.)
...
7. Tests (`tests/test_models_manager.py`):
   ...
   - whisper.cpp arg plumbing: existing `TestWhisperCppModelResolution`
     ...
     `-l de` present with an override and absent with "auto". The CLI
     transcribe path gets one test asserting it routes through
     `effective_language` (stub backend records the kwarg).
...
7. Test-style notes (repo law, per `tests/test_model_download.py`):
   - Prefer a pristine per-test cache root: `monkeypatch.setenv(
     "XDG_CACHE_HOME", str(tmp_path))` redirects every `paths` helper
     (env read lazily per call) — the `cache` fixture pattern in
     `tests/test_model_download.py:39-42`. The session-wide conftest
     isolation already keeps you off the real cache either way.
   - Cleanup/delete assertions use exact contents: `list(dir.iterdir()) ==
     [expected]` / `== []` — assert the decoy is gone AND every sibling is
     byte-identical after a refused delete.
   - For synthetic parakeet entries, monkeypatch
     `model_catalog.PARAKEET_CATALOG` with a fixture model (the `pk`
     fixture pattern, `tests/test_model_download.py:146-163`); note
     `tests/test_model_catalog.py` asserts catalog entries as exact key
     sets — this plan adds NO catalog metadata fields (sizes are computed
     from disk), so those assertions must stay untouched.
